...
const insertQuery=`
    ...
    `;const values=[username,password,name,logo_image,location,phone_number,email,website_url,0,description,account_type];await db.query(insertQuery,values);res.redirect("/login")}catch(err){console.error(err);res.redirect("/register")}});app.get("/list/:type",async(req,res)=>{try{const type=req.params.type;const result=await db.query(`
...
return acc},{});const itemsArray=Object.values(accounts);res.render("list.ejs",{type:type,is_user:req.session.is_user,companies:itemsArray})}catch(error){console.error("Error fetching accounts:",error);res.redirect("login")}});app.get("/admin",async(req,res)=>{if(!req.session.is_admin){return res.send("false")}
try{const[posts_data,data]=await Promise.all([get_all_data(),get_users_data()]);const accounts=data.rows.reduce((acc,row)=>{if(!acc[row.id]){acc[row.id]={id:row.id,name:row.name,username:row.username,location:row.location,logo_image:[],email:row.email,account_type:row.account_type}}
if(row.logo_image){acc[row.id].logo_image.push(row.logo_image.toString('base64'))}
...
const{companyName,location,number,details,title,accountID,list_type}=req.body;try{await db.query('BEGIN');const result=await db.query("INSERT INTO posts (account_name, location, description, account_id, post_title, post_type) VALUES ($1, $2, $3, $4, $5, $6) RETURNING post_id",[companyName,location,details,accountID,title,list_type]);const postId=result.rows[0].post_id;const imageValues=imageArray.map(image=>[image.data,postId]);const insertImagesQuery=`
    ...
    `;const insertImagesParams=imageValues.flat();await db.query(insertImagesQuery,insertImagesParams);await db.query('COMMIT');res.redirect("/")}catch(err){await db.query('ROLLBACK');console.error("Error uploading data:",err);res.redirect("/login")}});app.post("/uploaded",async(req,res)=>{try{const{upload_type}=req.body;const validTypes=['company','contractor','freelancer'];if(!validTypes.includes(upload_type)){return res.status(400).send("Invalid upload type.")}
const query=`
      SELECT 
        ci.id, 
        ci.name, 
...
const combinedData={...data,images:images};res.render('post.ejs',{data:combinedData,type:type,comments:comments,account_id:req.session.account_id})}catch(error){console.error("Error fetching details:",error);res.redirect("/login")}}});app.get("/account/:type/:id",async(req,res)=>{const type=req.params.type
...
const messages=await db.query('SELECT * FROM messages WHERE (sender_id = $1) OR (receiver_id = $1) ORDER BY timestamp ASC',[req.session.account_id]);function check(messages){if(messages.rows&&messages.rows.length>0){const lastMessage=messages.rows[messages.rows.length-1];if(lastMessage&&lastMessage.read_status!==undefined){return lastMessage.read_status}}
return!1}
res.render("uers-profile.ejs",{accounts:accounts,posts:posts,id:req.session.account_id,newM:check(messages),able_to_rate:able_to_rate})}else{const accounts=(await get_one_account(id))
const posts=await get_account_posts(id)
...
const accounts=Array.from(accountsMap.values());res.render('chats.ejs',{accounts,id:req.session.account_id})}catch(err){console.error(err);res.redirect('/login')}});server.listen(3000,()=>{console.log('Server is running on port 3000')})