...
const combinedData={...data,images:images};res.render('post.ejs',{data:combinedData,type:type,comments:comments,account_id:req.session.account_id})}catch(error){res.redirect("/login")}}});app.get("/account/:type/:id",async(req,res)=>{const type=req.params.type;const id=req.params.id;if(type==="delete"){try{await db.query('DELETE FROM messages WHERE sender_id = $1 OR receiver_id = $1;',[id])
await db.query(`DELETE FROM replies WHERE account_id = $1`,[id]);await db.query(`DELETE FROM comments WHERE account_id = $1`,[id]);await db.query(`DELETE FROM post_images WHERE post_id IN (SELECT post_id FROM posts WHERE account_id = $1)`,[id]);await db.query(`DELETE FROM posts WHERE account_id = $1`,[id]);await db.query(`DELETE FROM account WHERE id = $1`,[id]);res.redirect("/")}catch(error){res.redirect("/login")}}else{let able_to_rate=!0;if(type==="viewafterrating"){able_to_rate=!1}
...
return!1};res.render("uers-profile.ejs",{accounts:accounts,posts:posts,type:req.session.type,id:req.session.account_id,newM:check(messagesResult),able_to_rate:able_to_rate})}catch(error){res.redirect("/login")}}});app.post("/rating/:id",async(req,res)=>{const id=req.params.id
