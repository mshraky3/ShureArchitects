...
return acc},{});res.render("list.ejs",{type:type,is_user:req.session.is_user,companies:accounts})}catch(error){console.error("Error fetching accounts:",error);res.redirect("login")}});app.get("/admin",async(req,res)=>{if(!req.session.is_admin){return res.send("false")}
